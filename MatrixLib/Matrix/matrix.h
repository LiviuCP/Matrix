...
Matrix<DataType>::DIterator::DIterator(DataType** pMatrixPtr,
    ...
    : m_pMatrixPtr{pMatrixPtr}
{
    CONSTRUCT_FORWARD_DITERATOR(m_DiagonalNr, m_DiagonalSize, m_DiagonalIndex, nrOfMatrixRows, nrOfMatrixColumns, first, second, isRelative);
}
...
Matrix<DataType>::ReverseDIterator::ReverseDIterator(DataType** pMatrixPtr,
    ...
    : m_pMatrixPtr{pMatrixPtr}
{
    CONSTRUCT_REVERSE_DITERATOR(m_DiagonalNr, m_DiagonalSize, m_DiagonalIndex, nrOfMatrixRows, nrOfMatrixColumns, first, second, isRelative);
}
...
Matrix<DataType>::MIterator::MIterator(DataType** pMatrixPtr,
    ...
    , m_NrOfMatrixColumns{nrOfMatrixColumns}
{
    CONSTRUCT_FORWARD_MITERATOR(m_DiagonalNr, m_DiagonalSize, m_DiagonalIndex, m_NrOfMatrixColumns, nrOfMatrixRows, first, second, isRelative);
}
...
Matrix<DataType>::ReverseMIterator::ReverseMIterator(DataType** pMatrixPtr,
    ...
    , m_NrOfMatrixColumns{nrOfMatrixColumns}
{
    CONSTRUCT_REVERSE_MITERATOR(m_DiagonalNr, m_DiagonalSize, m_DiagonalIndex, m_NrOfMatrixColumns, nrOfMatrixRows, first, second, isRelative);
}
...
{
    CHECK_ERROR_CONDITION(firstMatrix.m_NrOfColumns != secondMatrix.m_NrOfColumns, Matr::errorMessages[Matr::Errors::COLUMN_DOES_NOT_EXIST]);
                                 ...
                                 Matrix<DataType>::size_type matrixColumnNr)
{
    ...
    {
        std::swap(m_pBaseArrayPtr[m_RowCapacityOffset + rowNr][columnNr], matrix.m_pBaseArrayPtr[matrix.m_RowCapacityOffset + matrixRowNr][matrixColumnNr]);
    }
}
                                ...
                                Matrix<DataType>::size_type matrixRowNr)
{
    ...
    const size_type c_AbsRowNr{m_RowCapacityOffset + rowNr};
    const size_type c_MatrixAbsRowNr{matrix.m_RowCapacityOffset + matrixRowNr};

    if (&matrix == this && c_AbsRowNr != c_MatrixAbsRowNr)
    {
        // exchanging row pointers is enough if rows belong to same matrix (constant time swap)
        std::swap(m_pBaseArrayPtr[c_AbsRowNr], m_pBaseArrayPtr[c_MatrixAbsRowNr]);
    }
    else if (&matrix != this)
    {
        std::swap_ranges(m_pBaseArrayPtr[c_AbsRowNr], m_pBaseArrayPtr[c_AbsRowNr] + m_NrOfColumns, matrix.m_pBaseArrayPtr[c_MatrixAbsRowNr]);
    }
}
                                   ...
                                   Matrix<DataType>::size_type matrixColumnNr)
{
    ...
    if (columnNr != matrixColumnNr || &matrix != this)
    {
        for(size_type absRowNr{m_RowCapacityOffset}; absRowNr != m_RowCapacityOffset + m_NrOfRows; ++absRowNr)
        {
            std::swap(m_pBaseArrayPtr[absRowNr][columnNr], matrix.m_pBaseArrayPtr[absRowNr][matrixColumnNr]);
        }
    }
}
                                     ...
                                     Matrix<DataType>::size_type matrixColumnNr)
{
    ...
    const size_type c_AbsRowNr{m_RowCapacityOffset + rowNr};
    const size_type& c_MatrixColumnNr{matrixColumnNr}; // for readability purposes (see below)
...
template <typename DataType>
void Matrix<DataType>::copy(const Matrix<DataType>& matrix,
...
{
    CHECK_ERROR_CONDITION(&matrix == this, Matr::errorMessages[Matr::Errors::CURRENT_MATRIX_AS_ARG]);
    CHECK_ERROR_CONDITION(nrOfRows < 0 || nrOfColumns < 0 || matrixRowNr < 0 || matrixColumnNr < 0 || rowNr < 0 || columnNr < 0,
                          Matr::errorMessages[Matr::Errors::NEGATIVE_ARG]);
    CHECK_ERROR_CONDITION(matrixRowNr + nrOfRows > matrix.m_NrOfRows ||
                          matrixColumnNr + nrOfColumns > matrix.m_NrOfColumns ||
                          rowNr + nrOfRows > m_NrOfRows ||
                          columnNr + nrOfColumns > m_NrOfColumns,
                          Matr::errorMessages[Matr::Errors::INVALID_ELEMENT_INDEX]);


    const size_type c_MatrixAbsStartingRowNr{matrix.m_RowCapacityOffset + matrixRowNr};
    const size_type c_AbsStartingRowNr{m_RowCapacityOffset + rowNr};

    for (size_type matrixAbsRowNr{c_MatrixAbsStartingRowNr}, absRowNr{c_AbsStartingRowNr}; absRowNr != c_AbsStartingRowNr + nrOfRows; ++absRowNr, ++matrixAbsRowNr)
    {
        std::copy_n(matrix.m_pBaseArrayPtr[matrixAbsRowNr] + matrixColumnNr, nrOfColumns, m_pBaseArrayPtr[absRowNr] + columnNr);
    }
}

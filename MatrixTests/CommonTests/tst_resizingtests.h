...
#define TEST_RESERVE_RESIZE_CHECK_MATRIX_SIZE_AND_CAPACITY(matrix, requiredNrOfRows, requiredNrOfColumns,              \
    ...
    CHECK_MATRIX_SIZE_AND_CAPACITY(                                                                                    \
        ...
        requiredRowCapacityOffset, requiredColumnCapacityOffset,                                                       \
        "Reserving/resizing failed, number of rows or columns of the matrix is not correct!",                          \
        "Reserving/resizing failed, capacity of the matrix is not correct!",                                           \
        "Reserving/resizing failed, capacity offset of the matrix is not correct!")

#define TEST_SHRINK_TO_FIT_CHECK_MATRIX_SIZE_AND_CAPACITY(matrix, requiredNrOfRows, requiredNrOfColumns)               \
    CHECK_MATRIX_SIZE_AND_CAPACITY(matrix, requiredNrOfRows, requiredNrOfColumns, requiredNrOfRows,                    \
                                   requiredNrOfColumns, requiredNrOfRows == 0 ? std::nullopt : std::optional{0u},      \
                                   requiredNrOfColumns == 0u ? std::nullopt : std::optional{0u},                       \
...
#define TEST_INSERT_COLUMN_CHECK_MATRIX_SIZE_AND_CAPACITY(matrix, requiredNrOfRows, requiredNrOfColumns,               \
                                                          requiredRowCapacity, requiredColumnCapacity,                 \
                                                          requiredRowCapacityOffset, requiredColumnCapacityOffset)     \
    CHECK_MATRIX_SIZE_AND_CAPACITY(matrix, requiredNrOfRows, requiredNrOfColumns, requiredRowCapacity,                 \
                                   requiredColumnCapacity, requiredRowCapacityOffset, requiredColumnCapacityOffset,    \
...
#define TEST_MATRIX_RESERVE(matrixType, primaryMatrix)                                                                 \
    ...
    primaryMatrix.reserve(requestedRowCapacity, requestedColumnCapacity);                                              \
                                                                                                                       \
    TEST_RESERVE_RESIZE_CHECK_MATRIX_SIZE_AND_CAPACITY(primaryMatrix, matrix.getNrOfRows(), matrix.getNrOfColumns(),   \
                                                       expectedRowCapacity, expectedColumnCapacity,                    \
                                                       expectedRowCapacityOffset, expectedColumnCapacityOffset);       \
                                                                                                                       \
    QVERIFY2(primaryMatrix == matrix, "Reserving/resizing failed, the matrix does not have the correct values!");
...
#define TEST_MATRIX_RESIZE_WITH_DEFAULT_CAPACITY_AND_SET_NEW_VALUES(matrixType)                                        \
    ...
    matrix.resize(requestedRowsCount, requestedColumnsCount, fillValue);                                               \
                                                                                                                       \
    TEST_RESERVE_RESIZE_CHECK_MATRIX_SIZE_AND_CAPACITY(matrix, requestedRowsCount, requestedColumnsCount,              \
                                                       expectedRowCapacity, expectedColumnCapacity,                    \
                                                       expectedRowCapacityOffset, expectedColumnCapacityOffset);       \
                                                                                                                       \
...
#define TEST_MATRIX_RESERVE_AND_RESIZE_WITH_SET_NEW_VALUES(matrixType)                                                 \
    ...
    matrix.resize(requestedRowsCount, requestedColumnsCount, fillValue);                                               \
                                                                                                                       \
    TEST_RESERVE_RESIZE_CHECK_MATRIX_SIZE_AND_CAPACITY(matrix, requestedRowsCount, requestedColumnsCount,              \
                                                       requestedRowCapacity, requestedColumnCapacity,                  \
                                                       expectedRowCapacityOffset, expectedColumnCapacityOffset);       \
                                                                                                                       \
    QVERIFY2(matrix == expectedMatrix, "Reserving/resizing failed, the matrix does not have the correct values!");

#define TEST_MATRIX_RESERVE_AND_RESIZE_CHECK_RETAINED_ELEMENT_VALUES(matrixType, matrix,                               \
                                                                     expectedRetainedElementsMatrix)                   \
    ...
    {                                                                                                                  \
        for (Matrix<matrixType>::size_type columnNr{0u}; columnNr < expectedRetainedElementsMatrix.getNrOfColumns();   \
             ++columnNr)                                                                                               \
        {                                                                                                              \
            areRetainedValuesCorrect =                                                                                 \
                areRetainedValuesCorrect &&                                                                            \
                (matrix.at(rowNr, columnNr) == expectedRetainedElementsMatrix.at(rowNr, columnNr));                    \
        }                                                                                                              \
    }                                                                                                                  \
                                                                                                                       \
    QVERIFY2(areRetainedValuesCorrect, "Reserving/resizing failed, the retained element values are not correct!");
...
#define TEST_MATRIX_INSERT_ROW_SET_VALUE(matrixType)                                                                   \
    ...
    matrix.insertRow(insertPosition, insertedRowValue);                                                                \
                                                                                                                       \
    TEST_INSERT_ROW_CHECK_MATRIX_SIZE_AND_CAPACITY(                                                                    \
        matrix, referenceMatrix.getNrOfRows(), referenceMatrix.getNrOfColumns(), expectedRowCapacity,                  \
        expectedColumnCapacity, expectedRowCapacityOffset, expectedColumnCapacityOffset);                              \
                                                                                                                       \
...
#define TEST_MATRIX_INSERT_COLUMN_SET_VALUE(matrixType)                                                                \
    ...
    matrix.insertColumn(insertPosition, insertedColumnValue);                                                          \
                                                                                                                       \
    TEST_INSERT_COLUMN_CHECK_MATRIX_SIZE_AND_CAPACITY(                                                                 \
        matrix, referenceMatrix.getNrOfRows(), referenceMatrix.getNrOfColumns(), expectedRowCapacity,                  \
        expectedColumnCapacity, expectedRowCapacityOffset, expectedColumnCapacityOffset);                              \
                                                                                                                       \
...
#define TEST_MATRIX_ERASE_ROW(matrixType)                                                                              \
    ...
    matrix.eraseRow(erasePosition);                                                                                    \
                                                                                                                       \
    TEST_ERASE_ROW_CHECK_MATRIX_SIZE_AND_CAPACITY(                                                                     \
        matrix, expectedMatrix.getNrOfRows(), expectedMatrix.getNrOfColumns(), expectedRowCapacity,                    \
        expectedColumnCapacity, expectedRowCapacityOffset, expectedColumnCapacityOffset);                              \
                                                                                                                       \
...
#define TEST_MATRIX_ERASE_COLUMN(matrixType)                                                                           \
    ...
    matrix.eraseColumn(erasePosition);                                                                                 \
                                                                                                                       \
    TEST_ERASE_COLUMN_CHECK_MATRIX_SIZE_AND_CAPACITY(                                                                  \
        matrix, expectedMatrix.getNrOfRows(), expectedMatrix.getNrOfColumns(), expectedRowCapacity,                    \
        expectedColumnCapacity, expectedRowCapacityOffset, expectedColumnCapacityOffset);                              \
                                                                                                                       \
    QVERIFY2(matrix == expectedMatrix,                                                                                 \
